...
LOG_FILE_PATH = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
...
class TextFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        log_message = f'{timestamp} - {record.filename} - {record.funcName}- {level} - '
        log_message = log_message + f'{message}'
        if uuid != '-':
...
class AgentLogger:
    ...
    def set_file_handle(self,
        ...
        file_log_formatter = get_formatter(
            os.getenv(LOG_FILE_FORMAT, 'normal').lower())
        current_date = datetime.now().strftime("%Y%m%d")
        info_file_path = os.path.join(log_dir, f"app_{current_date}.log")
